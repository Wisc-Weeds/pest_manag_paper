...
readxl::read_excel("droplet_size_data.xlsx") %>% 
  ...
  unite("trt", c(solution, nozzle), sep = "-", remove = FALSE) -> droplet
```
...
cld4 %>% 
  ...
  annotate("text", x = 5, y = 0.1, label = "There are differences \namong all pairwise trt  \ncomparisons (P <.0001) \nin figures A, B and C", size = 9, hjust = 0, lineheight = 0.3,
           color = 1, fontface = "italic") +
  labs(x = NULL, y = "Relative spam", color = NULL) +
  scale_color_viridis_d(option = "H") +
...
#           color = 1, fontface = "italic") +
  labs(x = NULL, y = "% drifable fines < 200 µm", color = NULL) +
  scale_color_viridis_d(option = "H") +
...
fig1 + fig2 + fig3 +
  ...
  theme(plot.tag = element_text(size = 25, hjust = 0, vjust = 0))

ggsave("Figure 4.png", width = 9, height = 3)
```
